...
$(document).ready(function(){
	...
	$("#addusers").on("click",function(){
		window.location.href=context+"/service/demo1/job/adds?userid="+userid1;
	});
...
function initTable(){
	oTable = $("#userList").dtable({
					//显示“正在加载”的图标
					"processing":true,
					"ajax":context + "/service/demo1/job/findPlan?userid="+userid1+"&week="+week1,
					...
					"columnDefs":[
						  ...
						  },
						{
							...
							"render":function(data,type,full){
								...
								+'<a id="del" onclick="del('+"'"+ full.id+ "'"+')" href="#">删除</a>'+'<span>&nbsp;&nbsp</span>'
								+'<a id="edit" onclick="lookinfo('+"'"+ full.id+ "'"+')" href="#">查看</a></div>'
							}
						}
					]
...
function query(week){
	if ((week == "" || week == null)) {
		week=getWeekNumber();
		console.log(week);
		oTable.ajax.url(context + "/service/demo1/job/findPlan?userid="+userid1+"&week="+week).load();
		return week1=week;
	} else {
...
function del(id){
	$.ajax({
		...
		async:false,
		success:function(){
			oTable.ajax.url(context+"/service/demo1/job/findPlan?userid="+userid1+"&week="+week1).load();
		},
		error:function(data){
			alert(data);
		}
...
function forUpdate(id){
	window.location.href=context+"/service/demo1/job/edits?id="+id+"&userid="+userid1;
}
